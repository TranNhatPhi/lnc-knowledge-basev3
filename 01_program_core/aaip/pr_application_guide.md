---
access_level: chatbot
chunk_strategy: standard
content_hash: b088806cdd67f1cc
file_role: pr_application_guide
lang: en
last_updated: '2026-06-30'
priority_weight: 1
program: AAIP
province: AB
retrieval_strategy: direct
source_id: aaip_after_nominated
source_url: https://www.alberta.ca/aaip-rural-entrepreneur-stream-after-you-are-nominated
topic: post_nomination
version: '1.0'
---

## Apply for permanent residence
To obtain permanent residence status, you must apply for Permanent Residence to Immigration, Refugees and Citizenship Canada (IRCC) with your AAIP nomination certificate and supporting documents. You must ensure you maintain your legal status in Canada.
IRCC completes a review of your application for permanent residence including health, security and criminal reviews. IRCC may request any additional information that it deems appropriate at any time during the application process, such as your Business Performance Agreement.
IRCC is responsible for assessing each nominee’s eligibility for permanent residence. AAIP is not responsible for IRCC’s decision to approve or refuse an application for permanent residence and or to issue a Confirmation of Permanent Residence.
### Step 1. Review your nomination package
Your AAIP nomination certificate is available under the “View Decision Letters” in the AAIP portal. If you have an authorized representative, notify them of the decision on your Business Application.
Read everything in the package carefully. Contact the program officer who assessed your application if you find mistakes on your nomination.
Make a copy of your nomination for your personal records.
IRCC requires you to provide a copy of this nomination certificate with your application for permanent residence. You may also be asked to provide a copy of this nomination certificate to renew your work permit.
### Step 2. Get your permanent residence application ready
After you get your AAIP nomination, you must apply to IRCC as soon as possible. Your nomination is only valid for 6 months; you must apply to IRCC before it expires.
You may qualify to have your AAIP nomination extended. A $150 fee applies for all requests for nomination extensions.
The sooner you apply for permanent residence, the sooner you can get an Acknowledgement of Receipt (AOR) of your application from IRCC, and an IRCC file number. Your AOR and IRCC file number are very important if you need to extend your work permit.
Visit the [IRCC website](https://www.canada.ca/en/immigration-refugees-citizenship/services/application/application-forms-guides/application-package-provincial-nominees.html) to get all the information you need to apply for permanent residence.
IRCC will not accept your application if it is incomplete or you have not paid the required fees.
### Step 3. Submit your application online
[Apply online](https://prson-srpel.apps.cic.gc.ca/en/login) to submit your application for permanent residence.